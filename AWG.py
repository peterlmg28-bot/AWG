...
def 查詢_AWG(awg):
    	cd /Users/peterlin/Documents/augment-projects/AWGdata = awg_data.get(awg)
    if not data:
        return f"AWG {awg} 不存在於資料表中。"
    return (
        f"AWG {awg} 規格:\n"
        f"外徑: {data['直徑_mm']} mm ({data['直徑_inch']} inch)\n"
        f"截面積: {data['截面積_mm2']} mm2\n"
        f"電阻值: {data['電阻_ΩKm']} Ω/Km\n"
        f"正常電流: {data['正常電流_A']} A\n"
...
def 反查_AWG(直徑值, 單位="mm"):
    ...
    for awg, data in awg_data.items():
        ...
        if 差距 < 最小差距:
            最小差距 = 差距
            最接近awg = awg
    result = f"最接近的 AWG 是: AWG {最接近awg} (差距 {最小差距:.4f} {單位})\n"
...
def 切換模式():
    if mode_var.get() == "AWG查詢":
        awg_frame.pack(fill="x", padx=10, pady=5)
        diameter_frame.pack_forget()
    else:
        diameter_frame.pack(fill="x", padx=10, pady=5)
        awg_frame.pack_forget()

if __name__ == "__main__":
    root = tk.Tk()
    root.title("AWG 查詢系統")

    mode_var = tk.StringVar(value="AWG查詢")
    result_text = tk.StringVar()

    mode_frame = ttk.Frame(root)
    mode_frame.pack(fill="x", padx=10, pady=5)
    ttk.Label(mode_frame, text="選擇模式:").pack(side="left")
    ttk.Radiobutton(mode_frame, text="AWG查詢", variable=mode_var, value="AWG查詢", command=切換模式).pack(side="left")
    ttk.Radiobutton(mode_frame, text="直徑反查", variable=mode_var, value="直徑反查", command=切換模式).pack(side="left")

    awg_frame = ttk.Frame(root)
    ttk.Label(awg_frame, text="AWG編號:").pack(side="left")
    awg_entry = ttk.Entry(awg_frame)
    awg_entry.pack(side="left")
    ttk.Button(awg_frame, text="查詢", command=查詢模式).pack(side="left")

    diameter_frame = ttk.Frame(root)
    ttk.Label(diameter_frame, text="直徑值:").pack(side="left")
    diameter_entry = ttk.Entry(diameter_frame)
    diameter_entry.pack(side="left")
    unit_var = tk.StringVar(value="mm")
    ttk.Label(diameter_frame, text="單位:").pack(side="left")
    ttk.Combobox(diameter_frame, textvariable=unit_var, values=["mm", "inch"], width=5).pack(side="left")
    ttk.Button(diameter_frame, text="反查", command=反查模式).pack(side="left")

    切換模式()

    result_label = ttk.Label(root, textvariable=result_text, background="#f0f0f0", anchor="w", justify="left")
